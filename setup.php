...
function plugin_customfields_getDatabaseRelations()
{
	...
	{
		$relations=array();
		$query="SELECT * FROM glpi_plugin_customfields_fields WHERE hidden=0 AND data_type='dropdown' ORDER BY device_type";
		$result=$DB->query($query);
		while ($data=$DB->fetch_assoc($result))
		{
...
function plugin_customfields_getSearchOption()
{
	...
	$query="SELECT f.*, dd.is_tree FROM glpi_plugin_customfields as cf, glpi_plugin_customfields_fields AS f ".
		" LEFT JOIN glpi_plugin_customfields_dropdowns AS dd ON dd.system_name=f.system_name ".
		" WHERE f.device_type=cf.device_type AND cf.enabled=1 AND f.hidden=0 AND f.data_type<>'sectionhead' ".
		" ORDER BY f.device_type, f.sort_order";
	$result=$DB->query($query);
	...
	while ($data=$DB->fetch_assoc($result))
	{
		...
		{
			$num = 5201; // Range 5200-5399 used by this plugin
		...
		if($data['data_type']=='dropdown')
		{
			...
			$sopt[$device_type][$num]['table']=$data['dropdown_table'];
			if($data['is_tree']==1)
				$sopt[$device_type][$num]['field']='completename';
			else
				$sopt[$device_type][$num]['field']='name';
			$sopt[$device_type][$num]['linkfield']=$data['system_name'];
			$sopt[$device_type][$num]['name']=$data['label'];
		}
		else
		{
			...
			if(CUSTOMFIELDS_GLPI_PATCH_APPLIED)
			{
				...
				$sopt[$device_type][$num]['table']=$table;
				$sopt[$device_type][$num]['field']=$data['system_name'];
				$sopt[$device_type][$num]['linkfield']='';
				$sopt[$device_type][$num]['name']=$data['label'];
				$sopt[$device_type][$num]['purpose']='log'; // an extra field used to clean search options
				$num++;

				// for mass update
				$sopt[$device_type][$num]['table']=$table2;
				$sopt[$device_type][$num]['field']=$data['system_name'];
				$sopt[$device_type][$num]['linkfield']=$data['system_name'];
				$sopt[$device_type][$num]['name']=$data['label'];
				$sopt[$device_type][$num]['purpose']='update'; // an extra field used to clean search options
				$num++;
			}
			...
			$sopt[$device_type][$num]['table']=$table2;
			$sopt[$device_type][$num]['field']=$data['system_name'];
			$sopt[$device_type][$num]['linkfield']='ID';
			$sopt[$device_type][$num]['name']=$data['label'];
			$sopt[$device_type][$num]['purpose']='search'; // an extra field used to clean search options
		}
	}
...
{	
	...
	if (isset($_SESSION['glpi_plugin_customfields_installed']) && $_SESSION['glpi_plugin_customfields_installed']==1)
	{
		$query="SELECT * FROM glpi_plugin_customfields_fields WHERE data_type <> 'sectionhead';";
		$result=$DB->query($query);
		while ($data=$DB->fetch_assoc($result))
		{
			$type=5200 + $data['device_type']; // this plugin uses the range 5200-5399
