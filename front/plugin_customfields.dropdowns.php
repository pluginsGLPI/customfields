...
commonHeader($LANGCUSTOMFIELDS['Manage_Custom_Dropdowns'],$_SERVER['PHP_SELF'],'plugins','customfields');
...
elseif(isset($_POST['add']) && $haveright)
{
	...
	$label=($_POST['label'] !='') ? $_POST['label'] : $LANGCUSTOMFIELDS['Custom_Dropdown'];

	if ($system_name=='') {
		$system_name='dropdown_';
		$extra=1;
	}
	else
		$extra='';
...
echo '<a href="./plugin_customfields.config.php">'.$LANGCUSTOMFIELDS['Back_to_Manage'].'</a><br><br>';

echo '<form action="#" method="post">';
echo '<table class="tab_cadre" cellpadding="5">';
echo '<tr><th colspan="5">'.$LANGCUSTOMFIELDS['Manage_Custom_Dropdowns'].'</th></tr>';
echo '<tr>';
echo '<th>'.$LANGCUSTOMFIELDS['Label'].'</th>';
echo '<th>'.$LANGCUSTOMFIELDS['System_Name'].'</th>';
echo '<th>'.$LANGCUSTOMFIELDS['Uses_Entities'].'</th>';
echo '<th>'.$LANGCUSTOMFIELDS['Tree_Structure'].'</th>';
echo '<th></th>';
echo '</tr>';
...
while ($data=$DB->fetch_assoc($result)){
	...
	if($data['num_links']==0 && $haveright)
		...
		echo str_replace('NNN',$data['num_links'],$LANGCUSTOMFIELDS['Used_by_NNN_devices']);
	echo '</td></tr>';
}
if($haveright)
{
	...
	if($DB->numrows($result)>0)
		...
		echo $LANGCUSTOMFIELDS['no_dd_yet'];
	echo '</td></tr>';
}
echo '</table>';
echo '</form>';
...
if($haveright)
{
	...
	echo '<tr><th colspan="5">'.$LANGCUSTOMFIELDS['Add_New_Dropdown'].'</th></tr>';
	echo '<tr>';
	echo '<th>'.$LANGCUSTOMFIELDS['Label'].'</th>';
	echo '<th>'.$LANGCUSTOMFIELDS['System_Name'].'</th>';
	echo '<th>'.$LANGCUSTOMFIELDS['Uses_Entities'].'</th>';
